...
<div class="corr-container" bind:clientWidth={w} bind:clientHeight={h}>
  ...
  <!-- viewBox="0 0 
        {visData.xAxis.length*35 + padding.left + padding.right}
        {visData.yAxis.length*35 + padding.top + padding.bottom}" -->
  ...
  >
    ...
    <g class="total-x">
      {#each visData.cateSepX as pac}
        ...
        <text
          x={columnSpacing((pac.line[0] + pac.line[1]) / 2 + 0.5)}
